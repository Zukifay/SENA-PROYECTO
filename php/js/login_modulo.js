...
document.addEventListener("DOMContentLoaded", () => {
    ...
    inputUser.addEventListener("input", () => {
        inputUser.value = inputUser.value.replace(/\s{2,}/g, " ").trimStart();
        validarCampo(userNameRegex, inputUser, "El nombre debe tener entre 4 y 40 caracteres, y contener letras. No puede ser solo espacios o solo números.");
    });

    inputEmail.addEventListener("input", () => {
        inputEmail.value = inputEmail.value.trimStart();
        validarCampo(emailRegex, inputEmail, "Correo inválido.");
    });

    inputPass.addEventListener("input", () => {
        inputPass.value = inputPass.value.trimStart();
    ...
    inputEmail.addEventListener("blur", async () => {
        ...
        try {
            const res = await fetch("../register_db.php", {
                method: "POST",
                headers: {
